...
contract MockENSBulkRegistrar is IBulkRegistrarController {
    ...
    uint256 REGISTRATION_FEE = 0.01 ether;
    uint256 SHORT_NAME_REGISTRATION_FEE = 0.1 ether;
    ...
    function _registerName(Registration calldata registration) internal returns (uint256) {
        ...
        return bytes(registration.name).length > 4 ? REGISTRATION_FEE : SHORT_NAME_REGISTRATION_FEE;
    }
}
